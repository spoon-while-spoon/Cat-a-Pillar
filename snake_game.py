...
# Fonts
try:
    retro_font = pygame.font.Font('PressStart2P.ttf', 20)  # Ensure this font file is available
except:
...
def get_highscore_file_path(mode='classic', level=None):
    ...
    if sys.platform == 'darwin':
        ...
        base_dir = os.path.expanduser('~/Library/Application Support/SnakeMF')
    elif sys.platform.startswith('win'):
        # Windows directory
        base_dir = os.path.join(os.environ.get('APPDATA'), 'SnakeMF')
    else:
        # For other operating systems, e.g., Linux
        base_dir = os.path.expanduser('~/.snakemf')
